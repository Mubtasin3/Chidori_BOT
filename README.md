...
## v5 fixes
- Uses the official BgUtils PO-token provider for modern YouTube extraction.
- Uses yt-dlp with curl_cffi support and modern YouTube clients.
- Fixes the aiosqlite `threads can only be started once` error by using the connection context manager correctly.
- Render Docker starts the PO-token provider and the bot in the same container.
